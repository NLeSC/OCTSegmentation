...
%   $Revision: 1.1.6.25.4.1 $  $Date: 2008/02/01 15:07:21 $
...
switch whichcall
    ...
    case 'reportValuesLogic',
        varargout{1} = reportValuesLogic(varargin{:});
...
%********************************************************************
function out = valueHasAppropriateIndexing(name, value) %#ok
out = false;
if length(name) >= 3
...
function [nameForAssign, toReport, startRow, startColumn, ...
    ...
    classID, customClassDesciption, dims, recurse] = reportValuesLogic(...
...
fullHeight = size(value, 1);
fullWidth  = size(value, 2);
classID = getMLArrayRefClass(value);
customClassDesciption = '';
dims = length(size(value));

trueEndRow = size(value, 1);
if (endRow == -1)
    endRow = trueEndRow;
else
    endRow = min(endRow, trueEndRow);
end
trueEndColumn = size(value, 2);
if (endColumn == -1)
...
signed = true;
if isinteger(toReport)
    if isa(toReport, 'uint8') || isa(toReport, 'uint16') || ...
            isa (toReport, 'uint32') || isa(toReport, 'uint64')
        signed = false;
    end
end
...
if isempty(toReport)
        ...
        if isreal(toReport)
            if isscalar(toReport)
                if isfloat(toReport)
                    ...
                    ca = ComplexScalarFactory.valueOf(convertIntegerType(toReport, signed), signed);
                end
            else
                if isfloat(toReport)
                    ca = ComplexArrayFactory.valueOf(toReport);
                else
                    ca = ComplexArrayFactory.valueOf(convertIntegerType(toReport, signed), signed);
                end
            end
        else
            if isscalar(toReport)
    ...
    elseif iscell(toReport)
        if uitableUsage
            ...
            % cell array editor wants truncated strings, but not quoted.
    ...
    elseif isstruct(toReport) && numel(toReport) > 1
        ...
        ca = createReadOnlyValue(toReport);
       try
           if ~isempty(meta.class.fromName(customClassDescription))
               isMCOS = true;
           end
       catch
       end
...
vmi = ValueMetaInfo(classID, getAttributes(varargin{2}), customClassDescription, isMCOS, dims, ...
    fullWidth, fullHeight);
...
%********************************************************************
function converted = convertIntegerType(value, signed)
converted = value;
if ~signed
    if isa(value, 'uint8')
...
out = ValueMetaInfo(type, attributes, clazz, isMCOS, numOfDims, width, height);
...
function [whosInformation, propertyAttributes] = ...
    whosInformationForProperties(mcos, values, names, writablesBasedOnContext)
...
%********************************************************************
function [names, writables] = getCurrentContextPropsAndPerms(mcos, ...
...
%********************************************************************
function out = structToWhosInformation(mcos)
import com.mathworks.mlwidgets.workspace.*;
