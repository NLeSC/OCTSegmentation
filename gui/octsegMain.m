...
function hMenFileOpenFileCallback(hObject, eventdata)
    [filename,pathname] = uigetfile( ...
        {'*.vol;*.oct;*.list;*.pgm; *.tif; *.jpg; *.bmp; *.png;' , 'All OCT Files (*.vol, *.oct, *.list, *.pgm, *.tif, *.jpg, *.bmp, *.png)';...
        '*.vol', 'Heidelberg Engineering RAW-Files (*.vol)'; ...
        '*.list', 'OCT Volume Image List (*.list)'; ...
        '*.pgm; *.tif; *.jpg; *.bmp; *.png', 'Image Files (*.pgm, *.tif, *.jpg, *.bmp, *.png)'; ...
        '*.oct', 'OCTSEG RAW-Files (*.oct)'; ...
        '*.csv', 'Scan saved in CSV File (*.csv)'},...       
        'Select OCT file');
    ...
    elseif(ActDataDescriptors.Header.ScanPattern == 2)
        ...
        if (numDescriptor == FILETYPE.IMAGE) || (numDescriptor == FILETYPE.CSV)
            switchMode(3);
        else
            switchMode(2);
        end
